...
directions-flutter의 관리자 페이지가 이 라우터로 분류(이름 + 대표 이미지)를
만들고 지운다. 새 기록 이름은 이 분류 목록 중 하나로 Gemini가 자동 분류된다
(app/services/gemini_classify.py). 전부 X-Admin-Token 헤더로 보호한다 — 로그인한
일반 사용자도 호출할 수 없다(app/auth.py의 require_admin, 신호등 적재와 같은 패턴).
"""
